...
t_IO_connection* crear_IO_connection(const char* nombre, tipo_interfaz_t tipo, int fd) {
    ...
    nueva_conexion->cola_procesos_bloqueados = list_create();
...
void* obtener_proceso_bloqueado(t_IO_connection* conexion) {
    t_list* cola_bloqueados_io = obtener_cola_procesos_bloqueados(conexion);
    ...
    void* proceso_bloqueado = list_remove(cola_bloqueados_io, 0);
...
bool agregar_proceso_bloqueado(t_IO_connection* conexion, void* proceso) {
    ...
    pthread_mutex_lock(mutex_blocked);
    list_add(cola_bloqueados_io, proceso);
    pthread_mutex_unlock(mutex_blocked);
...
bool tiene_procesos_bloqueados(t_IO_connection* cliente_io) 
{
    ...
    pthread_mutex_lock(mutex_blocked);
    t_list* cola_bloqueados_io = obtener_cola_procesos_bloqueados(cliente_io);
    is_empty = list_is_empty(cola_bloqueados_io);
    pthread_mutex_unlock(mutex_blocked);
